  ...
  if(!empty($all_userss))
    {
	  ...
	  if($users_level == "Moderator" || $users_level == "Administrator") {
		...
		continue;
          ...
          if($_GET['edit'] == TRUE and $_GET['userid'] == $users_arr[0]) echo '<td><a href="'.$PHP_SELF.'?id=modules/leaderA&action=view_leader&start_from='.$here.'&userid='.$users_arr[0].'">';
  ...
  if($start_from > 0)
    {
      ...
      $npp_nav .= '<td class="col-lg-6 col-md-6" class="con1" align="left"><a href="'.$PHP_SELF.'?id=modules/leaderA&action=view_leader&start_from='.$previous.'&users_per_page='.$users_per_page.'"><< Previous</a></td>';
      $tmp = 1;
    }
  ...
  if(count($all_userss) > $users_per_page)
    {
      ...
      for($j=1;$j<=$enpages_count;$j++)
        {
          if($enpages_start_from != $start_from) { $enpages .= '<a href="'.$PHP_SELF.'?id=modules/leaderA&action=view_leader&start_from='.$enpages_start_from.'&users_per_page='.$users_per_page.'"> '.$j.' </a>'; }
  ...
  if(count($all_userss) > $i)
    {
      ...
      $npp_nav .= '<td class="col-lg-6 col-md-6" class="con1" align="right"><a href="'.$PHP_SELF.'?id=modules/leaderA&action=view_leader&start_from='.$i.'&users_per_page='.$users_per_page.'">Next '.$how_next.' >></a></td>';
    }
